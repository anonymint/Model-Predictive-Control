 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      // actuation is at time t
      AD<double> delta0 = vars[delta_start + t - 1];
      AD<double> a0 = vars[a_start + t - 1];

      // since it's 3 order degree and psides0 will calculate of derivative on 3rd polynomial
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // push value of x,y,x1,y2,x2,y2... until n to result before return for visualization
  for (int i=0; i < N-1; i++) {
